...
app.post('/identify', async (req, res) => {
    ...
    try {
        ...
        const sorted = allRelated.sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());
...
function formatResponse(primary, all) {
    const emails = Array.from(new Set([primary.email, ...all.map((c) => c.email)])).filter(Boolean);
    const phoneNumbers = Array.from(new Set([primary.phoneNumber, ...all.map((c) => c.phoneNumber)])).filter(Boolean);
